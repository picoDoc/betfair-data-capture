...
getChances:{[mktid;bucket]
	...
	/ join on metadata
	chances lj 2!select sym, selectionId, eventTypeName, competitionName, marketName, eventName, selectionName from select by sym,selectionId from metadata}
...
getMid:{[mktid]
	...
	/ join on metadata
	mid lj 2!select sym, selectionId, eventTypeName, competitionName, marketName, eventName, selectionName from select by sym,selectionId from metadata}